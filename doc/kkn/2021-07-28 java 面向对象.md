...
2、编译main函数所在的文件，其他依赖会自动寻找进行编译，如果*.java的版本比 *.class的高，会重新编译。



构造器

与类名同名，在构造Employee类的对象时，构造器会运行，以便将实例域初始化为所希望的状态。 构造器与其他的方法有一个重要的不同。构造器总是伴随着new操作符的执行被调用，而不能对一个已经存在的对象调用构造器来达到重新设置实例域的目的。如：
